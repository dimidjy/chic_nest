...
namespace Drupal\app_core\Plugin\rest\resource;
...
/**
 * Provides a cart item resource with GET support.
 *
 * @RestResource(
 *   id = "app_cart_item",
 ...
 * )
 */
class CartItemResource extends CartResourceBase {
  ...
  /**
   * Constructs a new CartItemResource object.
   *
  ...
  /**
   * GET a specific order item from a cart.
   *
   ...
   *   The response.
   */
  public function get(OrderInterface $commerce_order, OrderItemInterface $commerce_order_item) {
    ...
    if (!in_array($commerce_order_item->id(), $order_item_ids)) {
      return new ModifiedResourceResponse(['error' => 'Order item does not belong to this order'], 404);
    }
    
    return new ModifiedResourceResponse($commerce_order_item, 200);
  }
